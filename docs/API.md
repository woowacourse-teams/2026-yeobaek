...
- 목록은 응답 순서대로 표시한다. 향후 처음 보는 `appliedSort` 문자열이 오더라도 목록 표시를
  실패시키지 않는다. 방문 목록은 최근 방문순이며 재방문하면 맨 앞으로 이동한다.
- 목록 항목의 `publicRoomId`로 공개방에 진입하고, `book.bookId`로 기존 모임 생성 API를 호출한다.
