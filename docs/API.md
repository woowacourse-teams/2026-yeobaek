...
- 참여 정보는 삭제하지 않고 `DISABLED`로 변경하며, 기존 댓글·작성자 정보·진도는 보존한다.
...
- 탈퇴한 모임은 목록에서 제외되며 `memberCount`는 활성 참여자만 집계한다.
...
- `members`: 활성 참여자만 참여 시각 오름차순으로 반환한다. `mine`은 요청자(`X-Member-Id`) 본인 여부 (댓글의 `mine`과 동일 규약).
