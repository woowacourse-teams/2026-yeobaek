...
이 문서의 `gh` 명령은 GitHub CLI를 사용할 수 있을 때의 참조 구현이다. GitHub CLI가 없으면
현재 인증된 GitHub REST·GraphQL API나 연결 도구로 같은 조회와 변경을 수행할 수 있다. 이 경우에도
각 단계의 실행 순서, 입력값, 충돌 판정과 사후 검증 항목을 동일하게 적용한다. 특정 도구의 부재
자체는 blocker가 아니며, 인증·권한 또는 동등한 조회·변경·검증 수단이 없을 때 중단한다.
...
- 현재 GitHub 인증 사용자가 5장의 백엔드 개발자 목록에 없다.
...
PR 제출 직전에 현재 GitHub 인증 사용자를 확인한다. GitHub CLI를 사용할 수 있으면 다음 명령을
실행한다.
...
제출자로 추정하지 않는다. 다른 GitHub API나 연결 도구를 사용할 때도 인증된 사용자의 login을
직접 조회해 같은 목록과 대조한다.
...
아래 `gh` 명령 대신 다른 GitHub API나 연결 도구를 사용하면 같은 값을 명시하고 같은 응답
필드를 확인한다.
...
생성 또는 갱신 직후 다음 명령을 실행한다. 다른 GitHub API나 연결 도구를 사용했다면 같은
필드와 review comment 전체를 조회한다.
